...
import 'package:apartments_manager_main/fetures/residents/screens/residents_adding_form.dart';
...
//------------------------------------------------------------------------------Fetch Vacant Rooms
  fetchVacantRooms() async {
    try {
      ...
      List<ApartmentModel> allRooms = await roomController.fetchData();
      ...
      if (vacantRooms.isNotEmpty) {
        ...
        for (ApartmentModel room in vacantRooms) {
          String roomNumber = room.apartmentNo.toString();
    ...
    try {
      ...
      if (editingResidnt!.roomNo == int.parse(selectedRoom!)) {
        ...
        try {
          ...
          final ApartmentModel? oldRoom = await roomController.fetchSingleRoom(
          ...
          final ApartmentModel? newRoom = await roomController.fetchSingleRoom(
            roomId: selectedRoomId!,
          );
...
//------------------------------------------------------------------------------fetching RoomId
  String findRoomByRoomNo(int roomNumber) {
    ...
    ApartmentModel? room = vacantRooms.firstWhere(
        (room) => room.apartmentNo == roomNumber,
        orElse: () => ApartmentModel.empty());
    return room.id!;
  }
